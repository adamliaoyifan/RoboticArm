...
## 六、真实 CAD STL → Gazebo（独立）

从 CAD 导出的外部 STL（如 [`realcontainer.STL`](realcontainer.STL)）直接生成 Gazebo 模型：**visual 保留完整网格**，**collision 用简化 decimate mesh**（默认约 8000 三角）。不依赖点云拟合或 `collision_boxes.json`。

### 配置与运行

```bash
cp config/real_container_gazebo.yaml.example config/real_container_gazebo.yaml
python3 scripts/generate_real_container_gazebo.py -c config/real_container_gazebo.yaml
```
...
roslaunch luggage_gazebo sim_world.launch \
  spawn_suitcases:=false \
  scene_tf_config:=$(rospack find luggage_description)/config/scene_tf.yaml.example
```

spawn 位姿和 `gazebo.container_model` 由 `scene_tf.yaml` 控制；container semantic bounds 也保存在同一个 `container:` 块中。

### 坐标系对齐

真实 CAD STL 的原点、开口朝向可能与点云拟合的 `measured_container.yaml` 不一致。首次加载请在 Gazebo 中检查：

- 底面是否贴地（脚本已锚定 `z_min ≈ 0`；必要时调整 `scene_tf.yaml` 的 `container_link` translation）
- 开口是否朝向机械臂工作区（mesh rotation 应烘焙进 STL；运行时保持 `container_link` rotation 为 0）
- 与 `airport_container_measured` 对比外形与尺寸（本模型 AABB 约 1.57 × 2.12 × 2.07 m）

若碰撞不稳定（非 watertight mesh），可增大 `collision_triangles` 或回退到第二节参数化 box 碰撞流程。

模块：[`generate_real_container_gazebo.py`](scripts/generate_real_container_gazebo.py)。
