#!/usr/bin/env bash
# Two more C1 scored runs after exp_nodump_refine (same vectorized refine, no dump).
set +u
set -o pipefail
...
if pgrep -f '/opt/ros/humble/bin/ros2 launch luggage_gazebo' >/dev/null; then
  ...
  pgrep -af '/opt/ros/humble/bin/ros2 launch luggage_gazebo'
  exit 1
fi
...
run_one() {
  ...
  mkdir -p "$out/gate4"
  rm -f "$out/STOP"
  local record="$LAUNCH_PARAMS ($name; vectorized _refine_rectangle; no dump-dir)"
...
if ! run_one exp_nodump_r2; then echo STREAK_FAIL; exit 1; fi
if ! run_one exp_nodump_r3; then echo STREAK_FAIL; exit 1; fi
echo STREAK_DONE
exit 0
