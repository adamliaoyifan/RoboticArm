...
class BinPacker:
    ...
    def _has_support(self, candidate, occupied, floor_z=0.0):
        if candidate["min_z"] <= floor_z + self._clearance:
            return True
        bottom_area = (
    ...
    def _candidate_mins(
            self, occupied, inner_l, inner_w, inner_h, box,
            floor_z=0.0, ceiling_z=None):
        width, depth, height = box.width, box.depth, box.height
        ceiling_z = inner_h if ceiling_z is None else float(ceiling_z)
        # container_link origin is the bottom-face CENTER (X/Y centered,
        # Z=0 at the exterior base), so the usable interior is
        # X in [-inner_l/2, +inner_l/2], Y in [-inner_w/2, +inner_w/2],
        # Z in [floor_z, ceiling_z]. X used to start
        # at 0.0 with range [0, inner_l] (treating the origin as the -X wall
        # corner), which offset every slot by +inner_l/2 and let slots past
        # the +X wall land OUTSIDE the container. Now X mirrors Y.
        xs = [-inner_l * 0.5] + [occ["max_x"] + self._clearance for occ in occupied]
        ys = [-inner_w * 0.5] + [occ["max_y"] + self._clearance for occ in occupied]
        zs = [floor_z] + [occ["max_z"] + self._clearance for occ in occupied]
        for z in self._unique_sorted(zs, floor_z):
            if z + height > ceiling_z + 1e-6:
                continue
            for y in self._unique_sorted(ys, -inner_w * 0.5):
                if y < -inner_w * 0.5 - 1e-6 or y + depth > inner_w * 0.5 + 1e-6:
                    continue
                for x in self._unique_sorted(xs, -inner_l * 0.5):
                    if x < -inner_l * 0.5 - 1e-6 or x + width > inner_l * 0.5 + 1e-6:
                        continue
                    yield x, y, z
    ...
    def handle_compute(self, req):
        ...
        inner_l, inner_w, inner_h = container_inner_dimensions(scene_config)
        floor_z = container_inner_floor_z(scene_config)
        ceiling_z = container_inner_ceiling_z(scene_config)
        ...
        for index, (x, y, z) in enumerate(
            self._candidate_mins(
                occupied, inner_l, inner_w, inner_h, box,
            ...
            if (
                    self._intersects(candidate, occupied)
                    or not self._has_support(candidate, occupied, floor_z=floor_z)):
                continue
    ...
    def handle(self, req):
        ...
        current = self._current_box_from_param()
        if current is None:
            # Boxes are now sized continuously, so there is no "standard" size
            # to stand in for a missing one. Substituting one would plan a slot
            # for a box that does not exist.
            return GetNextSlotResponse(
                slot=SlotSpec(), success=False,
                message="no current box on /luggage/current_box")
...
def main():
    rospy.init_node("bin_packer", log_level=resolve_log_level())
    packer = BinPacker()
