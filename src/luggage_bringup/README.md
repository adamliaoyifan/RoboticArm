...
- `exploration_mode:=none|smart|constrained_view|nbv|interior_probe` — Cargo exploration before inspect (default `smart`).
- `exploration_config:=...` — geometry-view, probe, and NBV settings (`exploration.yaml.example`).
...
Expected active flow with exploration: `ResetObserve → SyncScene → SpawnCurrentBox → ExploreCargo → ExploreCargo → InspectContainer → ComputePlacement → PlanPick → ExecPick → PlanPlace → ExecPlace → UpdateOccupancy → SpawnCurrentBox`.
...
| `cargo_exploration_planner` | `plan_next_cargo_view` | Geometry-driven smart/probe views, constrained views, or greedy NBV candidates |
...
| Layer | Color | Meaning |
|-------|-------|---------|
| Inner Cargo wireframe | cyan | `scene_tf.yaml` container semantic interior bounds |
...
Orchestrator flow (`exploration_mode:=smart`):
...
`LoadTaskStatus.state` may be: `Idle`, `ResetObserve`, `SyncScene`, `SpawnCurrentBox`, `ExploreCargo`, `InspectContainer`, `ComputePlacement`, `Detect`, `PlanPick`, `ExecPick`, `PlanPlace`, `ExecPlace`, `UpdateOccupancy`.
