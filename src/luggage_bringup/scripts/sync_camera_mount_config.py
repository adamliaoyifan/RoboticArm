...
Camera now mounts on eef_mount_adapter (not elfin_link6 directly)."""
...
def _write_xacro(parent_link, fixed_xyz, fixed_rpy):
    ...
    body = (
        '<?xml version="1.0"?>\n'
        "<!-- D435 side mount on eef_mount_adapter. fixed.* from tune_joints via sync_camera_mount_config.py -->\n"
...
def main():
    ...
    parent = mount.get("parent_link", "eef_mount_adapter")
