...
class TestEvaluateBagStub(unittest.TestCase):
    ...
    def test_lidar_full_archive(self):
        ...
        self.assertEqual(points.shape, (4,))
        self.assertEqual(
            points.dtype.names,
            ("x", "y", "z", "intensity", "tag", "line", "timestamp"))
        # Deskew inputs: per-point absolute-ns timestamps + span in index.
        self.assertGreater(float(points["timestamp"].max()),
                           float(points["timestamp"].min()))
        self.assertIn("scan_span_ms", rows[0])
        self.assertIn("ts_min_ns", rows[0])
        self.assertAlmostEqual(rows[0]["scan_span_ms"], 3.0, places=3)
        report = json.load(open(os.path.join(
            self.bag_out, "join_report.json")))
