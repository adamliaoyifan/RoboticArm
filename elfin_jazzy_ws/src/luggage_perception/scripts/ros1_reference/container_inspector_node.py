...
from box_catalog_utils import (  # noqa: E402
    box_catalog_entries,
...
class ContainerInspector:
    ...
    def __init__(self):
        ...
        self._scene_config = load_scene_tf_config(self._scene_tf_config_path)
        self._catalog = load_box_catalog(self._catalog_path, self._scene_config)
        self._model_sizes = {}
        ...
        self._require_free_slots = bool(rospy.get_param(
    ...
    def handle(self, req):
        ...
        if mode in ("depth", "fused"):
            if rospy.has_param("/luggage/cargo_map/stats"):
                ...
                if not self._allow_gt_fallback:
                    return InspectContainerResponse(
                        success=False,
                        message="INSPECT_MAP_MISSING strict mode=%s" % mode,
                        free_volume=0.0,
                        occupancy_ratio=0.0,
                        free_slots=[],
                    )
                rospy.logwarn(
                    "%s inspect requested but cargo map missing; GT fallback",
                    mode)

        if not free_slots and self._require_free_slots:
            return InspectContainerResponse(
                success=False,
...
def main():
    rospy.init_node("container_inspector", log_level=resolve_log_level())
