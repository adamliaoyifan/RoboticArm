...
class PlacementPlannerNode:
    ...
    def _fsm_candidates(self, surface, box_size):
        ...
        ems = EMS(usable_inner, min_useful_edge=min(self._smallest_box_size))
        ...
        score_candidates(
            candidates, model, ems, usable_inner, self._smallest_box_size,
    ...
    def handle_compute(self, req):
        ...
        if min(box_size) <= 0.0:
            box_size = self._box_size_from_param() or box_size
        candidates, message = self._compute(box_size)
