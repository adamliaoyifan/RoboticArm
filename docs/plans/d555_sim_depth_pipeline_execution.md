...
R2 is a lineage and certification boundary, not permission to reinterpret the
stream contract. Satisfying or waiving R2 does not change the simulated camera
path:

```text
Gazebo colour image + Gazebo 32FC1 metre depth + Gazebo CameraInfo
        |
        v
single-stream depth adapter to 16UC1 millimetres
        |
        v
preprocessor exact acquisition:
colour + aligned depth + colour CameraInfo + aligned-depth CameraInfo
        |
        v
consumer-local pixel selection, same-acquisition K, stamped TF
```

Simulation must not use D555 native depth, `/camera/d555/depth/color/points`,
Gazebo camera clouds, approximate pairing, latest-TF fallback, or hardware
compressed transport as the simulated RGB-D path. Passing DSIM-INTEGRATION
certifies the simulated depth-image pipeline only; it does not certify
PF-R10/PF-R7, hardware hand-eye validity, or D555 hardware stream quality.

Real-bag measurements in
`docs/status/evidence/d555_livox_bag_audit/20260917_1002/RESULT.md` are
diagnostic inputs for simulation parity. They establish that live D555 RGB and
aligned depth exact-match by header acquisition stamp; recorder log time has
millisecond jitter and is not the join clock.
...
If Livox is present in simulation, it is an independent Mid-360-like lidar
stream, not a cloud synthesized from D555 depth and not a required member of
the exact RGB-D acquisition. It publishes `/livox/lidar` as `PointCloud2` in
`livox_frame`, with its own lidar stamp and `deskewed=false` unless per-point
times plus FK interpolation are actually modeled. Do not claim `/livox/imu`
unless an IMU model with correct units exists.
...
When the Livox sim stream is enabled, also record its measured rate, frame,
point count, finite ratio, range p50/p95/max, and deskew state. These metrics
are observability for the optional lidar stream; they must not become an RGB-D
pairing dependency.
...
If the integration run includes Livox, report it separately from the camera
stream gate. The real Mid-360 bags measured about 20k points per scan at
10 Hz, with full-cloud nearest-neighbor spacing around 54 mm mean and
dominant-surface spacing around 20-25 mm mean. The current raster sim profile
may use a lower or higher configured density, but the Result must state the
configured sample grid and observed points/scan. Point count alone is not a
quality metric: surface monitoring should include finite ratio, nearest-neighbor
spacing, dominant-plane inlier ratio, point-to-plane residual mean/std,
intensity mean/variance when available, and whether the cloud was produced
after settle or with deskew.
...
- Do not synthesize Livox points from D555 depth or exact-pair Livox with RGB-D.
