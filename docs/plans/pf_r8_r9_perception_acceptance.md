...
(`support_ransac_ms` p50 126.0 -> 0.604 ms, `false_measured_height` 0 in every
    10|run). `gate4_short6` still fails intermittently, and
...
inside the frame (`[178,144,405,297]` at `t=59.467 s`), on a separate object.
    ...
    30|This is the already-documented `base_link` false positive
...
The damage is upstream of geometry:
    ...
    40|`DetectionTemporalGate.apply` sets
...
(n=120 in-region detections: min 0.042, p25 0.124, p50 0.240).
    50|
**Consequence for sequencing:** suppressing the false positive *alone* converts
those frames from a `top_surface_rate` miss into a `cargo = 0` frame, which is
then an `active_output_hz` miss. Suppression and hold repair must ship together
or the failure only relocates. This is why PF-R8 is one subtask, not two.
...
of frames are never emitted, with `raw_img->pre_rgb` p50 **248.7 ms** / p95
    60|428 ms. At the filter's topic boundary the exact join already succeeds for
...
`(now_hint - rgb_stamp) >= self.camera_slop_sec`, and `now_hint` is the max
    70|stamp over **all** buffers including `/joint_states` at 50 Hz. One joint
...
5. Missing data is flagged, not faked (`ObservationFlags`, never an empty array).
    90|6. `false_measured_height` must stay 0. A change that raises availability by
...
- **Pixel-space masking** (apply the mask to the organised depth image and
  ...
  float64 TF pass, and the filter's `(u,v)` reprojection). This is the largest
   100|  available win but changes the preprocessor's published contract and needs a
...
## 4. Subtasks
   ...
   110||     ID     | Owner agent/model | Depends on | Bounded scope | Acceptance | Required tests |
|---|---|---|---|---|---|
| PF-R8 | `claude/glm-5.3` | none | Cargo detection availability: suppress the static/border false positive and repair the temporal hold. Files: `luggage_perception/detection_temporal_gate.py`, `luggage_perception/semantic_segmenter.py`, `config/semantic_segmenter.yaml` | A1-A5 below | Focused unit tests + offline replay of the 155-frame capture + 16-pose sweep |
| PF-R9 | `claude/glm-5.3` | none | Preprocessor throughput and cloud-wait semantics. Files: `luggage_perception/sensor_preprocessor.py`, `scripts/sensor_preprocessor_node.py`, `config/sensor_preprocessor.yaml`, stats throttle in `scripts/semantic_point_filter_node.py` | B1-B6 below | Focused preprocessor unit tests + a >=60 s live stage probe |
| PF-R10 | `claude/glm-5.3` | PF-R8,PF-R9 | Integration: whole-chain gate4 re-baseline on one committed revision | C1-C3 below | `gate4_short6` x3 + PF-G6S lifecycle |

PF-R8 and PF-R9 touch disjoint files and may run in either order. Both take
`f34d917` as base. PF-R6 generation 3's own changes are still uncommitted
(`git_dirty_files` 183 in the gen-3 evidence); the same owner holds PF-R6, so
   120|**PF-R10 cannot pass until PF-R6 generation 3 is committed**, because C1 must
be measured on a real commit with `dirty=0`.

### PF-R8 acceptance

- **A1** A cargo detection is rejected unless it is plausibly the payload.
  The predicate is the owner's choice inside scope, but it MUST reject the
  captured false positive and MUST NOT reject the captured true detections.
  Candidate signals: bbox touching an image border, back-projection outside
  the pickup workspace XY extent already known to `top_support_estimator`, or
  static-across-spawns persistence. State the chosen predicate in the note.
   130|- **A2** `DetectionTemporalGate` treats "this frame saw cargo" as "saw an
...
- **A3** Offline replay of
  `docs/status/evidence/platform_free_height/2026-09-07_pfr6_gen3/failed_case_capture/failed_cases.jsonl`
  (155 frames): the 16 `DETECT_TOP_UNOBSERVABLE` frames produce a held
  suitcase mask consistent with their temporal neighbours, and do **not**
  become `cargo = 0` frames. Report the count for all three outcomes
  (accepted detection / held / empty) before and after.
   140|- **A4** The 16-pose sweep recall does not regress against the PF-R5 baseline,
  and `confidence_threshold` is unchanged unless the note argues the change.
...
- **B1** Record the missing measurement first: header-stamp period **versus**
  arrival-time period for `/camera/depth/points` and `/camera/color/image_raw`
   150|  over >= 60 s, in evidence. This decides whether gz drops frames or the
...
- **B3** Emitted-observation rate >= 0.8 x input `raw_img` rate, and
  `raw_img->pre_rgb` p50 <= 60 ms, on the same stage probe used for B1.
   160|- **B4** `cloud_ok` true on >= 95 % of emitted observations, and the filter's
  exact join succeeds on >= 95 % of preprocessed clouds with stale drops
  < 5 %.
- **B5** Cost reductions are in scope and expected: multi-threaded executor
  ...
  B3/B4 is acceptable; justify what was skipped.
   170|- **B6** Existing preprocessor unit tests pass, plus new tests for the split
...
- **C1** `gate4_short6`, 6 trials, 3 consecutive runs on one committed
  ...
  in **all three** runs. Two-of-three is not a pass; the failures being fixed
   180|  are intermittent.
- **C2** PF-G6S lifecycle unchanged: bounded queues and RSS, zero residual
  processes after `scripts/stop_sim.sh`.
...
- B5 touches concurrency. The `semantic_point_filter_node` comments record a
   200|  MultiThreadedExecutor + Reentrant group combination that spawned ~70 threads
...
## 6. Pointers
   ...
   210|- `docs/agents/reviews/2026-09-07_1956_pf-r6-gen3-current-issues-review.md`
- `docs/status/evidence/platform_free_height/2026-09-07_pfr6_gen3/CURRENT_ISSUES.md`
