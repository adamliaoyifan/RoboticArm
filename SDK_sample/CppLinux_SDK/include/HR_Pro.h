/*******************************************************************
 *	Copyright(c) 2020-2025 Company Name
 *  All rights reserved.
 *	
 *	FileName:HR_Pro.h
 *	Descriptio:C++SDK
...
*                                                                                            HRIF_EnterSafeGuard
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.9.0                       2025.1.16                             HRIF_ReadRobotType
 *                                                                                          HRIF_ClearCoControlExpandAxisPos            
 *                                                                                          HRIF_SetRotationVelocityControlMode
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.10.0                      2025.3.24                             HRIF_SetBaseInstallingAngle
 ...
 *                                                                                          HRIF_ReadForceData
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.11.0                     2025.3.25                              HRIF_StartWeaveWeld
 *                                                                                          HRIF_SetWeaveParams
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.12.0                     2025.3.25                              HRIF_LongMoveEvent
 *                                                                                          HRIF_ShortJogJ
 *                                                                                          HRIF_ShortJogL
 *                                                                                          HRIF_LongJogJ
 *                                                                                          HRIF_LongJogL
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.13.0                     2025.4.15                              HRIF_CalTCPOrt
 *                                                                                          HRIF_SetDftTCP
 *                                                                                          HRIF_ReadActCoord
 *                                                                                          HRIF_ReadActCoord_nJ
 * ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ 
 * V1.0.14.0                     2025.8.12                              HRIF_GetInverseDynamic
...
#define MaxJointNums 15
    ...
    #ifdef HANSROBOT_PRO_EXPORTS
        #define HANSROBOT_PRO_API __attribute__((dllexport))  
    #else  
        #define HANSROBOT_PRO_API __attribute__((dllimport))  
    #endif  
#else
    #define HANSROBOT_PRO_API  //此宏定义没有实际意义，只是为了在接口的命名形式上跟Windows版本保持一致，两边比较代码不至于出现太多的差异
#endif
...
/**
 *	@brief: SDK Automated Test Print Interface Functions / SDK自动化测试打印接口函数
  *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param bState : Set the status to false to disable printing, or to true to enable printing / 设置状态，false为关闭打印，true为开启打印
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int SetAutoTestState(unsigned int boxID, bool bState);
 ...
 *	@brief: Read the robot model / 读取机器人类型
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param RobotType : Robot Type / 机械臂设备类型
 *	@param MovelSet: MovelSet /机械臂本体型号系列，字符串，当前有“Elfin”，“S”等
 *	@param Axist6Type: The Sixth axis type/第6轴类型
 *  @param Axist7Type: The Seventh  axis type/第7轴类型
 *  @param Axist8Type: The Eighth  axis type/第8轴类型
 *  @param Axist9Type: The Ninth axis type/第9轴类型
 *  @param Axist10Type: The Tenth axis type/第10轴类型
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadRobotType(unsigned int boxID, int& RobotType,string&  MovelSet,int& Axist6Type,int& Axist7Type,int& Axist8Type,int& Axist9Type,int& Axist10Type);


//************************************************************************/
//**   Axis group control command interface / 轴组控制指令接口                                                             
//************************************************************************/
/**
...
/**
 *	@brief: Read whether the three-stage switch is turned on and the mode / 读取三段式开关是否开启以及模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nThreeStageEnable : Three stage switch  / 三段式按钮
 *                                                              0：colse/关闭
 *                                                              1：open/开启
 *  @param nThreeStageMode : Mode / 模式
 *                                                              0：FreeDrive/零力示教
 *                                                              1：Enable/使能
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadTriStageSwitch(unsigned int boxID, unsigned int rbtID,int&  nThreeStageEnable, int& nThreeStageMode);

/**
 *	@brief: Set three-stage switch and mode / 设置三段式开关以及模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nThreeStageEnable : Three stage switch  / 三段式按钮
 *                                                              0：colse/关闭
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetTriStageSwitch(unsigned int boxID, unsigned int rbtID, int  nThreeStageEnable, int nThreeStageMode);

/**
 *	@brief: Z-axis alignment / Z轴对齐
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param TcpName : Tcp Name  / 工具坐标系的名称
 *  @param UcsName : Ucs Name / 用户坐标系的名称
 *  @param bIsReached : Is the Z-axis aligned  / Z轴是否已经对齐
 *  @param dbJ1-dbJ6 : J1-J6 Joint Position / J1到J6轴的关节位置
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_MoveAlignToZ(unsigned int boxID, unsigned int rbtID, string TcpName, string UcsName,bool &bIsReached,double &dbJ1,double &dbJ2,double &dbJ3,double &dbJ4,double &dbJ5,double &dbJ6);
...
/**
 *	@brief: Start a task to bring the robot to Standby state from other states, replacing other multiple commands.  After the task is launched, the system checks the current state and sends related commands automaticallly. If the system can't return to Standby state within 30s, the task will end. / 启动任务，将机械臂从其它状态返回到Standby状态，以取代其它多条指令下发。任务启动后，系统会自动识别当前状态并自动发出相应的指令。如果30秒内仍不能返回到Standby状态，任务自动结束。
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @return : nRet=0 : Task launched successfully / 任务启动成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_XToStandby(unsigned int boxID, unsigned int rbtID);
...
/**
 *	@brief:Applies the specified script file / 应用指定的脚本文件
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param strScriptName : ScriptName / 脚本的名称
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SwitchScript(unsigned int boxID, unsigned int rbtID, string strScriptName);

/**
 *	@brief:Read the current application script file / 读取当前应用脚本文件
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param strScriptName : ScriptName / 当前应用的脚本名称
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadDefaultScript(unsigned int boxID, unsigned int rbtID, string &strScriptName);
...
/**
 *	@brief: Read all control box config digital input statuses / 读取电箱所有的控制数字输入状态
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param Val : Vector storing all config input (CI) values / 存储所有可配置数字输入(CI)状态的向量
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadAllBoxCI(unsigned int boxID, vector<int>& Val);

/**
 *	@brief: Read all control box general digital input statuses / 读取电箱所有的通用数字输入状态
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param Val : Vector storing all digital input (DI) values / 存储所有通用数字输入(DI)状态的向量
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadAllBoxDI(unsigned int boxID, vector<int>& Val);

/**
 *	@brief: Read all control box config digital output statuses / 读取电箱所有的可配置数字输出状态
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param Val : Vector storing all config output (CO) values / 存储所有可配置数字输出(CO)状态的向量
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadAllBoxCO(unsigned int boxID, vector<int>& Val);

/**
 *	@brief: Read all control box general digital output statuses / 读取电箱所有的通用数字输出状态
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param Val : Vector storing all digital output (DO) values / 存储所有通用数字输出(DO)状态的向量
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadAllBoxDO(unsigned int boxID, vector<int>& Val);
...
/**
 *	@brief: Set End Button / 设置末端按键状态
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nStatus : End button status, 1 for enabled, 0 for enabled / 末端按键启用开关, 1: 启用, 0: 禁用
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_EnableEndBTN(unsigned int boxID, unsigned int rbtID, int nStatus);

/**
 *	@brief: Set IO before the motion command reaches the target point / 在运动指令到达目标点位前设置IO
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_cdsSetIO(unsigned int boxID, unsigned int rbtID, int nEndDOMask, int nEndDOVal,int nBoxDOMask,int nBoxDOVal,int nBoxCOMask,int nBoxCOVal,int nBoxAOCH0_Mask,int nBoxAOCH0_Mode,int nBoxAOCH1_Mask,int nBoxAOCH1_Mode,double dbBoxAOCH0_Val,double dbBoxAOCH1_Val);
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : Whether to enable the speed limit constraint on the change rate of orientation angles for MoveC/L commands./ 是否开启MoveC/L指令对姿态角变化速度的上限约束
 *	                           0 : Restricted (default) mode / 受限(默认)模式
 *                             1 : Unrestricted mode / 不受限模式	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_SetRotationVelocityControlMode(unsigned int boxID, unsigned int rbtID,int nMode);

/**
 *	@brief: Read joint range and linear range / 读取关节范围和空间范围
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dMaxJ1-dMaxJ6 : Max joint angle / 最大关节角度
 *	@param dMinJ1-dMinJ6 : Min joint angle /最小关节角度
 *	@param dMaxX-dMaxRZ : Max range of XYZRXRYRZ / XYZ、RXRYRZ最大范围
 *	@param dMinX-dMinRZ : Min range of XYZRXRYRZ / XYZ、RXRYRZ最小范围
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_ReadMaxRange(unsigned int boxID, unsigned int rbtID,vector<double> &MaxJointRange,vector<double> &MinJointRange,vector<double> &MaxLinerRange,vector<double> &MinLinerRange);
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_ReadMaxRange_nJ(unsigned int boxID, unsigned int rbtID,vector<double> &MaxJointRange,vector<double> &MinJointRange,vector<double> &MaxLinerRange,vector<double> &MinLinerRange);
 ...
 *	@param nSafeGuard_Error: Error occurs when the two safeguard signals are different / 安全光幕回路有两路，当两路信号不相同时，则认为安全光幕回路有错误，这个则为1
 *	@param nSafeGuard：Stop the motion but never cutt off power supply when safeguard occurs / 安全光幕信号，发生安全光幕时，会停止机器人运动，并且不再接受运动指令，不会断本体供电
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Enter or exit safeguard status by force to realize soft EStop(emergency stop). Move will be stopped. System can be reset after EStop is canceled. / 强制进入或者取消进入安全光幕状态，以此方式实现软件急停，以及软急停后的恢复。强制进入安全光幕状态后，当前的运动会被停止掉。需要先取消强制，然后才能复位系统。
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_EnterSafeGuard(unsigned int boxID, unsigned int rbtID, int nFlag);
 ...
 *	@param nSafeGuard : Safty guard state / 安全光幕状态
...
/**
 *	@brief: Read current FSM / 读取当前状态机及描述
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Read FSM from CPS / 读取当前状态机
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Read point list as name / 读取系统中所有点位的名称列表
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param PointList : point list / 点位列表
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadPointList(unsigned int boxID, unsigned int rbtID, vector<string> &PointList);
...
/***
 *	@brief: Calculate inverse dynamics results / 计算动力学逆解结果
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *	@param dTorq_J1~6 : Joint torques calculated by inverse dynamics (Nm) / 通过逆动力学计算得到的各关节扭矩（牛・米）
 *
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_GetInverseDynamics(unsigned int boxID, unsigned int rbtID, double dPose_J1, double dPose_J2, double dPose_J3, double dPose_J4, double dPose_J5, double dPose_J6,
...
/**
 *	@brief: Read TCP list / 读取系统中所有工具坐标的名称列表
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param TCPList : vector of TCP list / 工作坐标的名称列表
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadTCPList(unsigned int boxID, unsigned int rbtID, vector<string> &TCPList);
...
/**
 *	@brief: Read UCS list / 读取系统中所有用户坐标的名称列表
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param TCPList : vector of UCS list / 工作坐标的名称列表
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadUCSList(unsigned int boxID, unsigned int rbtID, vector<string> &UCSList);
...
/**
 *	@brief: Set force free drive translation compliance and rotation compliance / 设置力控自由驱动平移柔顺度和旋转柔顺度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Linear : 平移柔顺度
 *	@param Angular : 旋转柔顺度
...
/**
 *	@brief: Set speed mode for force free drive / 设置力控自由驱动速度模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : speed mode, 0 as normal mode, 1 as slow mode, 2 as fast mode / 速度模式，0:正常模式，1:慢速精准模式，2:快速流畅模式, 3:焊接模式
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetFTFreeDriveSpeedMode(unsigned int boxID, unsigned int rbtID, int nMode);

/**
 *	@brief:Read force control calibration data / 读取力控自由驱动速度模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : speed mode, 0 as normal mode, 1 as slowmode, 2 as fast mode / 速度模式，0:正常模式，1:慢速精准模式，2:快速流畅模式, 3:焊接模式
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadFTFreeDriveSpeedMode(unsigned int boxID, unsigned int rbtID, int &nMode);

/**
 *	@brief: Set the force control free drive filter mode / 设置力控自由驱动滤波模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : speed mode, 0 as normal mode, 1 as slow mode, 2 as fast mode / 滤波模式:0关闭滤波，1开启输入侧低通滤波，2开启输出侧均值滤波
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetFTFreeDriveFilter(unsigned int boxID, unsigned int rbtID, int nMode);

/**
 *	@brief:Read the force control free drive filter mode / 读取力控自由驱动滤波模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : speed mode, 0 as normal mode, 1 as slowmode, 2 as fast mode / 滤波模式:0关闭滤波，1开启输入侧低通滤波，2开启输出侧均值滤波
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadFTFreeDriveFilter(unsigned int boxID, unsigned int rbtID, int &nMode);

/**
 *	@brief: Set the maximum value, minimum value of the tangential force in the x/y direction and the maximum speed of lifting / 设置x/y方向切向力最大值、最小值和上抬最大速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Set the orientation compensation force size and vector direction [x,y,z] in FreeDrive mode / 设置FreeDrive模式下的定向补偿力大小和矢量方向[x,y,z]
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
...
/**
 *	@brief: Set the six-dimensional force activation threshold (force and torque) / 设置六维力启动阈值（力与扭矩）
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dForceThreshold : 自由驱动力启动阈值，单位：N
 *	@param dTorqueThreshold : 自由驱动力矩启动阈值，单位：N.m
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetFTWrenchThresholds(unsigned int boxID, unsigned int rbtID, double dForceThreshold, double dTorqueThreshold);

/**
 *	@brief: Set the maximum linear speed and attitude angular speed of force free drive / 设置力控自由驱动最大直线速度及姿态角速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dMaxLinearVelocity : 自由驱动最大直线速度，单位：mm/s
 *	@param dMaxAngularVelocity : 自由驱动最大姿态角速度，单位：°/s
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetMaxFreeDriveVel(unsigned int boxID, unsigned int rbtID, double dMaxLinearVelocity, double dMaxAngularVelocity);

/**
 *	@brief: Read the end degrees of freedom of the force free drive / 读取力控自由驱动的末端自由度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nX-nRz : The available freedom parameters in free drive / 设置力控拖动下可开放的自由度
 *	
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadFTMotionFreedom(unsigned int boxID,unsigned int rbtID,int& nX,int& nY,int& nZ,int& nRx,int& nRy,int& nRz);

/**
 *	@brief: Set each degree of freedom (X/Y/Z/RX/RY/RZ) to force control the maximum distance of exploration / 设置各自由度（X/Y/Z/RX/RY/RZ）力控探寻最大距离
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Dis_X-Dis_RZ : 探寻最大距离，单位：mm
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetMaxSearchDistance(unsigned int boxID,unsigned int rbtID,double Dis_X,double Dis_Y,double Dis_Z,double Dis_RX,double Dis_RY,double Dis_RZ);

/**
 *	@brief: Set constant force control stability phase boundary / 设置恒力控稳定阶段边界
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Pos_X-Pos_RZ : 正方向(与探寻方向一致)最大距离，单位：mm
 *	@param Neg_X-Neg_RZ : 负方向(与探寻方向相反)最大距离，单位：mm
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetSteadyContactDeviationRange(unsigned int boxID,unsigned int rbtID,double Pos_X,double Pos_Y,double Pos_Z,double Pos_RX,double Pos_RY,double Pos_RZ,
                                                double Neg_X,double Neg_Y,double Neg_Z,double Neg_RX,double Neg_RY,double Neg_RZ);

/**
 *	@brief: Set current UCS / 设置力控用户坐标系
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dUcs_X~Rz : Pose of UCS / 需设置的用户坐标值
 *	
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_SetFTUCS(unsigned int boxID, unsigned int rbtID, double dUcs_X, double dUcs_Y, double dUcs_Z, double dUcs_Rx, double dUcs_Ry, double dUcs_Rz);

/**
 *	@brief: Read current UCS / 读取力控用户坐标系
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID :  Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dUcs_X~Rz : Pose of UCS / 读取的用户坐标值
 *	
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadFTUCS(unsigned int boxID, unsigned int rbtID, double& dUcs_X, double& dUcs_Y, double& dUcs_Z, double& dUcs_Rx, double& dUcs_Ry, double& dUcs_Rz);
 ...
 *	@param nFixedPosure : 0 for Not fixed posture and posture angle change less than 180 °, 1 for fixed orientation, 2 for Unsteady posture and posture angle change greater than 180 °
                        / 0:不固定姿态且姿态角变化小于180°，1:固定姿态圆弧运动，2:不固定姿态且姿态角变化大于180°
 ...
 *                                            / 弧长为 0 时，表示目标位姿由 endPose 决定，设置为 360° 则可跑整圆，设置为 720 则跑两圈圆
 *	@param dVelocity : Motion speed, mm/s / 运动速度，单位是毫米每秒
 *	@param dAcc : Motion acceleration, mm/(s^2) / 运动加速度，毫米每秒平方
 *	@param dRadius : Blending radius, mm / 过渡半径，单位毫米
 *	@param sTcpName : TCP name / 刀具坐标名称
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveC2(unsigned int boxID, unsigned int rbtID,
    double dAuxPos_X,double dAuxPos_Y,double dAuxPos_Z,double dAuxPos_Rx,double dAuxPos_Ry,double dAuxPos_Rz,
    double dEndPos_X,double dEndPos_Y,double dEndPos_Z,double dEndPos_Rx,double dEndPos_Ry,double dEndPos_Rz,
...
/**
 * @brief   Structure of parameters required for MovePathJOL motion points，This structure is specifically for the HRIF_MovePathJOL interface. / MovePathJOL运动点位所需参数的结构体，此结构体专用于HRIF_MovePathJOL接口
 */
typedef struct _MovePathJOLPointsData
{
    /**
     * @brief  Joint target position of axis 1 / 1轴的关节目标位置
     */
    double  dbJ1;
    /**
     * @brief  Joint target position of axis 2 / 2轴的关节目标位置
     */
    double  dbJ2;
    /**
     *@brief  Joint target position of axis 3 / 3轴的关节目标位置
     */
    double  dbJ3;
    /**
     * @brief  Joint target position of axis 4 / 4轴的关节目标位置
     */
    double  dbJ4;
    /**
     * @brief  Joint target position of axis 5 / 5轴的关节目标位置
     */
    double  dbJ5;
    /**
     * @brief  Joint target position of axis 6 / 6轴的关节目标位置
     */
    double  dbJ6;
    /**
    ...
    int nIsSetIO;
    /**
     * @brief  The EndDO to be modified is identified by bit/ 需要更改的EndDO按bit标识
     */
    int nEndDOMask;
    /**
     * @brief  The target state of each EndDO to be modified./ 各个需要更改的EndDO的目标状态
     */
    int nEndDOVal;
     /**
     * @brief   The BoxDO to be modified is identified by bit. / 需要更改的BoxDO按bit标识
     */
    int nBoxDOMask;
    /**
     * @brief   The target state of each BoxDO to be modified. / 各个需要更改的BoxDO的目标状态
     */
    int nBoxDOVal;
    /**
     * @brief   The BoxCO to be modified is identified by bit / 需要更改的BoxCO按bit标识
     */
    int nBoxCOMask;
    /**
     * @brief   The target state of each BoxCO to be modified / 各个需要更改的BoxCO的目标状态
     */
    int nBoxCOVal;
    /**
     * @brief   Indicator of whether BoxAOCH0 needs to be modified. / BoxAOCH0是否需要更改的标识
     */
    int nBoxAOCH0_Mask;
    /**
     * @brief   Mode / 模式
    ...
    int nBoxAOCH0_Mode;
    /**
     * @brief   Indicator of whether BoxAOCH1 needs to be modified. / BoxAOCH1是否需要更改的标识
     */
    int nBoxAOCH1_Mask;
     /**
     * @brief    Mode / 模式
    ...
    int nBoxAOCH1_Mode;
    /**
     * @brief    Corresponding analog value / 对应模拟量值
     */
...
/**
 *	@brief: Start MovePathJ for online implementation planning / 启动在线实施规划的MovePathJ
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dVel : Point index finished / 关节速度
 *  @param dAcc : The total number of points in the current trajectory movement / 关节加速度
 *  @param dTol : The total number of points in the current trajectory movement / 过滤参数
 * @param JOLPointsData :  MovePathJOL motion point data. MovePathJOLPointsData is a structure that contains various data required for setting the MovePathJOL motion points. / MovePathJOL运动点位数据。MovePathJOLPointsData是一个结构体，里面包含了MovePathJOL运动点位所需设置的各种数据
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_MovePathJOL(unsigned int boxID, unsigned int rbtID, int dVel, int dAcc, int dTol,vector<MovePathJOLPointsData> JOLPointsData);

/**
 *	@brief: Obtain the index number of the current point position of MovePathJOL motion and the total number of points in the trajectory motion / 获取MovePathJOL运动当前的点位索引号及轨迹运动所有点总数。
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nCurIndex : Point index finished / 当前轨迹运动到的点位索引
 *  @param nIndexNum : The total number of points in the current trajectory movement / 当前轨迹运动所有点位总数
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_GetMovePathJOLIndex(unsigned int boxID, unsigned int rbtID, int& nCurIndex, int& nIndexNum);

/**
 *	@brief:  / 设置空间点位轨迹参考关节坐标
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nCurIndex :  / 轨迹名字
 *  @param dJ1-dJ6 :  / 参考关节坐标
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetPathRefJoints(unsigned int boxID, unsigned int rbtID, string& strPathName, double dJ1, double dJ2, double dJ3, double dJ4, double dJ5, double dJ6);
...
/**
 *	@brief: 在SpeedL模式下，下发空间运动命令速度指令
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dXCmdVel~dZCmdVel : position velocity / 更新的空间运动命令直线速度，单位：mm/s
 *	@param dRxCmdVel~dRzCmdVel : angular velocity / 更新的空间运动命令姿态角速度，单位：°/s
 *  @param dLinearAcc : linear Acc / 线性加速度，单位：mm/s^2
 *  @param dAngularAcc : angular Acc / 角度加速度，单位：°/s^2
 *  @param dRuntime : 指令运行超过该时间，运动将会停止。单位：s
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SpeedL(unsigned int boxID, unsigned int rbtID, double dXCmdVel,double dYCmdVel,double dZCmdVel,double dRxCmdVel,
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_HRAppCmd(unsigned int boxID,string AppName, string sCmdName, string sParams);
 ...
 *	@param sCmdName: Command name / 命令名称
 *	@param sParams: Parameters list / 参数列表
 *	@param sResult: Result list / 结果列表
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_HRAppData(unsigned int boxID,string AppName, string sCmdName, string sParams, vector<string>& sResults);
...
/**
 *	@brief: Set Modbus configuration for end connection / 设置末端连接的 Modbus 配置。
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nBaudrate: Baud rate, 115200 as default / 波特率,默认值=115200
 *	@param nDataBit: Data bits, 8 as default / 数据位，默认值=8
 *	@param nParityBit: Parity bit (78: No parity; 79: Odd parity; 69: Even parity) / 校验位,78:无校验；79:奇校验；69:偶校验
 *	@param nStopBit: Stop bits, 1 as default / 停止位,默认值=1
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_SetEndModbusInfo(unsigned int boxID, unsigned int rbtID, int nBaudrate, int nDataBit, int nParityBit, int nStopBit);
...
/**
 *	@brief:Add a new safe plane/新增安全平面
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_AddSafePlane(unsigned int boxID, unsigned int rbtID,  string Name, string UcsName , int Mode, int Display, int Switch);

/**
 *	@brief:Delete Safe Plane/删除安全平面
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param Name: plane name/平面名称
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_DelSafePlane(unsigned int boxID, unsigned int rbtID,  string Name);


/**
 *	@brief:Get SafePlane List /获取安全平面列表
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param BordersName: plane name/平面名称
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadSafePlaneList(unsigned int boxID, unsigned int rbtID,  vector<string>  &BordersName);

/**
 *	@brief:Read SafePlane Info/读取安全平面信息
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param Name: plane name/平面名称
 *  @param UcsName:UCS Name/用户坐标系名称
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadSafePlane(unsigned int boxID, unsigned int rbtID,  string Name, string &UcsName , int &Mode, int &Display, int &Switch);

/**
 *	@brief:Update SafePlane Info/更新安全平面
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param Name: plane name/平面名称
 *  @param UcsName:UCS Name/用户坐标系名称
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_UpdateSafePlane(unsigned int boxID, unsigned int rbtID,  string Name, string UcsName , int Mode, int Display, int Switch);

/**
 *	@brief:Set the distance threshold when the virtual wall begins to generate damping /设置虚拟墙开始产生阻尼时的距离阈值
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *  @param dDepth: Threshold size/阈值大小
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetDepthThresholdForDampingArea(unsigned int boxID, unsigned int rbtID, double dDepth);



//************************************************************************/
//**    Interfaces for Extended Axis / 扩展轴类函数接口                                                             
//************************************************************************/
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint1~n max velocity / J1-Jn轴最大速度，单位[°/ s]
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetJointMaxVel_nJ(unsigned int boxID, unsigned int rbtID, JointsData joints);
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint1~n max acceleration /J1-Jn轴轴最大加速度，单位[°/ s^2]
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetJointMaxAcc_nJ(unsigned int boxID, unsigned int rbtID, JointsData joints);
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param jdMaxJoints : Max joint angle / 最大关节角度
 *	@param jdMinJoints : Min joint angle /最小关节角度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_SetMaxAcsRange_nJ(unsigned int boxID, unsigned int rbtID, JointsData jdMaxJoints, JointsData jdMinJoints);
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint command positions / 命令关节位置
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadCmdJointPos_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);

/**
 *	@brief: Read command current of joint / 读取命令关节电流
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint command current / 命令关节电流
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadCmdJointCur_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);

/**
 *	@brief: Read actual current of joint / 读取实际关节电流
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint actual current / 实际关节电流
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActJointCur_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);

/**
 *	@brief: Read actual joint positions / 读取实际关节位置
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint actual positions / 实际关节位置
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActJointPos_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);

/**
 *	@brief: Read command velocity of joint / 读取命令关节速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint command velocity / 命令关节速度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadCmdJointVel_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);

/**
 *	@brief: Read actual velocity of joint / 读取实际关节速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Joint actual velocity / 实际关节速度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActJointVel_nJ(unsigned int boxID, unsigned int rbtID, JointsData& joints);
 ...
 *	@param dTcp_X~Rz : TCP pose / dCoord所在的工具坐标
 *	@param dUcs_X~Rz : UCS pose / dCoord所在的用户坐标
 *	@param joints : Reference joint positions used for the selection in multiple solutions / 参考关节坐标，对于多个解用于选取接近的关节坐标位置
 *	@param dTargetJoints : Inverse solution of J1~Jn / 逆解
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_GetInverseKin_nJ(unsigned int boxID, unsigned int rbtID, double dCoord_X, double dCoord_Y, double dCoord_Z, double dCoord_Rx, double dCoord_Ry, double dCoord_Rz,
                 double dTcp_X, double dTcp_Y, double dTcp_Z, double dTcp_Rx, double dTcp_Ry, double dTcp_Rz,
                 double dUcs_X, double dUcs_Y, double dUcs_Z, double dUcs_Rx, double dUcs_Ry, double dUcs_Rz,
                 JointsData joints,
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : joint positions / 需要计算的关节坐标位置
 *	@param dTcp_X~Rz : TCP pose for the target / Target所对应的工具坐标
 *	@param dUcs_X~Rz : UCS pose for the target / Target所对应的用户坐标
 *	@param dTarget_X~Rz : pose in specified UCS and TCP / 指定用户坐标系和工具坐标系下的迪卡尔坐标位置
 *	
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */				 
HANSROBOT_PRO_API int HRIF_GetForwardKin_nJ(unsigned int boxID, unsigned int rbtID, JointsData joints,
                 double dTcp_X, double dTcp_Y, double dTcp_Z, double dTcp_Rx, double dTcp_Ry, double dTcp_Rz,
                 double dUcs_X, double dUcs_Y, double dUcs_Z, double dUcs_Rx, double dUcs_Ry, double dUcs_Rz,
                 double &dTarget_X, double &dTarget_Y, double &dTarget_Z, double &dTarget_Rx, double &dTarget_Ry, double &dTarget_Rz);
 ...
 *  @param nPointList : Using point saved in points list or not, 0 for not using,  1 for using / 是否使用点位列表的点位，0为不使用，1为使用
 *  @param dPos_X~Rz : Target pose / 空间目标位置
 *  @param dJointsPos : Target joint positions / 关节目标位置
 *  @param nrelMoveType : Relative move type, 0 for absolute value, 1 for superimposed value / 相对运动类型，0为绝对值，1为叠加值
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_WayPointRel_nJ(unsigned int boxID, unsigned int rbtID,
    int nType,
    int nPointList,
    double dPos_X, double dPos_Y, double dPos_Z, double dPos_Rx, double dPos_Ry, double dPos_Rz,
 ...
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dX~Rz : Target pose, invalid when IsUseJoint equals 1. Target joint positions is obtained through inversely solving from this pose when IsUseJoint equals 0 / 目标迪卡尔位置-当nIsUseJoint=1时无效，nIsUseJoint=0时，用此迪卡尔坐标作为目标位置，通过逆解计算得到关节坐标为目标关节坐标
 *	@param joints : Target joint positions when IsUseJoint equals 1; Reference joint positions for the inversely solving when IsUseJoint equals 0. / 目标关节位置-当nIsUseJoint=1时，使用此关节坐标作为目标关节坐标，nIsUseJoint=0时，此关节坐标仅作为计算逆解时选解的参考关节坐标
 *	@param sTcpName : Target TCP name, invalid when IsUseJoint equals 1, "TCP" is valid as default / 目标迪卡尔坐标所处的工具坐标系名称，与示教器页面的名称对应，当nIsUseJoint=1时无效，可使用默认名称”TCP”
 *	@param sUcsName : Target UCS name, invalid when IsUseJoint equals 1, "Base" is valid as default / 目标迪卡尔坐标所处的用户坐标系名称，与示教器页面的名称对应-当nIsUseJoint=1时无效，可使用默认名称”Base”
 *	@param dVelocity : Motion speed, mm/s or °/s / 运动速度，速度单位是毫米每秒，度每秒
 *	@param dAcc : Motion acceleration, mm/(s^2) or °/(s^2) / 运动加速度，毫米每秒平方，度每秒平方
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveJ_nJ(unsigned int boxID, unsigned int rbtID, double dX, double dY, double dZ, double dRx, double dRy, double dRz,
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius, 
                int nIsUseJoint,int nIsSeek, int nIOBit, int nIOState, string strCmdID);
 ...
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dX~Rz : Target pose / 目标迪卡尔位置
 *	@param joints : Reference joint positions near to the target pose / 关节位置-建议使用目标迪卡尔坐标附近的关节坐标值。
 *	@param sTcpName : TCP name / 目标迪卡尔坐标所处的工具坐标系名称
 *	@param sUcsName : UCS name / 目标迪卡尔坐标所处的用户坐标系名称
 *	@param dVelocity : Motion speed, mm/s or °/s / 运动速度，速度单位是毫米每秒，度每秒
 *	@param dAcc : Motion acceleration, mm/(s^2) or °/(s^2) / 运动加速度，毫米每秒平方，度每秒平方
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveL_nJ(unsigned int boxID, unsigned int rbtID, double dX, double dY, double dZ, double dRx, double dRy, double dRz,
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius,
                int nIsSeek, int nIOBit, int nIOState, string strCmdID);
 ...
 *	@param sUcsName : UCS name / 用户坐标名称
 *	@param sCmdID : Command ID / 命令ID
 *  @param joints : The reference joint position of the end point / 结束点参考关节位置
 *  @param nIsUseCurRefACS : Whether to use the reference joint Angle /  是否使用参考的关节角度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveC_nJ(unsigned int boxID, unsigned int rbtID,
                double dStartPos_X,double dStartPos_Y,double dStartPos_Z,double dStartPos_Rx,double dStartPos_Ry,double dStartPos_Rz,
                double dAuxPos_X,double dAuxPos_Y,double dAuxPos_Z,double dAuxPos_Rx,double dAuxPos_Ry,double dAuxPos_Rz,
                double dEndPos_X,double dEndPos_Y,double dEndPos_Z,double dEndPos_Rx,double dEndPos_Ry,double dEndPos_Rz,
                int nFixedPosure,int nMoveCType,double dRadLen,double dVelocity, double dAcc, 
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Max velocity of J1~Jn / J1-Jn轴最大运动速度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadJointMaxVel_nJ(unsigned int boxID, unsigned int rbtID, JointsData &joints);

/**
 *	@brief: Read joint max jerk / 读取关节最大加加速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Max jerk of J1~Jn / J1-Jn轴最大加加速度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadJointMaxJerk_nJ(unsigned int boxID, unsigned int rbtID, JointsData &joints);

/**
 *	@brief: Read joint max acceleration / 读取关节最大加速度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : Max acceleration of J1~Jn / J1-Jn轴最大加速度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadJointMaxAcc_nJ(unsigned int boxID, unsigned int rbtID, JointsData &joints);

/**
 *	@brief: Read point pose, joint positions, UCS and TCP by point name / 根据点位名称读取其位置信息
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param joints : joint positions / 关节坐标
 *	@param dX~Rz : pose / 笛卡尔坐标
 *	@param dTcp_X~Rz : TCP pose / TCP坐标
 *	@param dUcs_X~Rz : UCS pose / 用户坐标
 *
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadPointByName_nJ(unsigned int boxID, unsigned int rbtID, string strPointName,
    JointsData &joints,
    double& dX, double& dY, double& dZ, double& dRx, double& dRy, double& dRz,
    double& dTcp_X, double& dTcp_Y, double& dTcp_Z, double& dTcp_Rx, double& dTcp_Ry, double& dTcp_Rz,
    double& dUcs_X, double& dUcs_Y, double& dUcs_Z, double& dUcs_Rx, double& dUcs_Ry, double& dUcs_Rz);
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActPos_nJ(unsigned int boxID, unsigned int rbtID,
                    double &dX, double &dY, double &dZ, double &dRx, double &dRy, double &dRz,
                    JointsData &joints,
                    double &dTcp_X, double &dTcp_Y, double &dTcp_Z, double &dTcp_Rx, double &dTcp_Ry, double &dTcp_Rz,
                    double &dUcs_X, double &dUcs_Y, double &dUcs_Z, double &dUcs_Rx, double &dUcs_Ry, double &dUcs_Rz);
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_WayPointEx_nJ(unsigned int boxID, unsigned int rbtID, int nMoveType,
                    double dX, double dY, double dZ, double dRx, double dRy, double dRz,                        
                    JointsData joints,
                    double dTcp_X, double dTcp_Y, double dTcp_Z, double dTcp_Rx, double dTcp_Ry, double dTcp_Rz,
                    double dUcs_X, double dUcs_Y, double dUcs_Z, double dUcs_Rx, double dUcs_Ry, double dUcs_Rz,
 ...
 *	@param dEndPos_X~Rz : Target pose, invalid when IsUseJoint equals 1. Target pose is obtained through inverse kinematics from this pose when IsUseJoint equals 0 / 目标迪卡尔位置-当nIsUseJoint=1时无效，nIsUseJoint=0时，用此迪卡尔坐标作为目标位置，通过逆解计算得到关节坐标为目标关节坐标
*	@param dAuxPos_X~Rz : Target auxiliary pose invalid when IsUseJoint equals 1, used as the middle point when nMoveType equals 2. Target pose is obtained through inverse kinematics from this pose when IsUseJoint equals 0 / 目标迪卡尔位置-当nIsUseJoint=1时无效，nIsUseJoint=0时，用此迪卡尔坐标作为目标位置，通过逆解计算得到关节坐标为目标关节坐标                                
 *	@param joints : Target joint positions when IsUseJoint equals 1; Reference joint positions for the inverse kinematics when IsUseJoint equals 0. / 目标关节位置-当nIsUseJoint=1时，使用此关节坐标作为目标关节坐标，nIsUseJoint=0时，此关节坐标仅作为计算逆解时选解的参考关节坐标
 *	@param sTcpName : Target TCP name, invalid when IsUseJoint equals 1, "TCP" is valid as default / 目标迪卡尔坐标所处的工具坐标系名称，与示教器页面的名称对应，当nIsUseJoint=1时无效，可使用默认名称”TCP”
 *	@param sUcsName : Target UCS name, invalid when IsUseJoint equals 1, "Base" is valid as default / 目标迪卡尔坐标所处的用户坐标系名称，与示教器页面的名称对应-当nIsUseJoint=1时无效，可使用默认名称”Base”
 *	@param dVelocity : Motion speed, mm/s or °/s / 运动速度，速度单位是毫米每秒，度每秒
 *	@param dAcc : Motion acceleration, mm/(s^2) or °/(s^2) / 运动加速度，毫米每秒平方，度每秒平方
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_WayPoint2_nJ(unsigned int boxID, unsigned int rbtID, int nMoveType,
                double dEndPos_X, double dEndPos_Y, double dEndPos_Z, double dEndPos_Rx, double dEndPos_Ry, double dEndPos_Rz,   
                double dAuxPos_X, double dAuxPos_Y, double dAuxPos_Z, double dAuxPos_Rx, double dAuxPos_Ry, double dAuxPos_Rz,                       
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius, 
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_WayPoint_nJ(unsigned int boxID, unsigned int rbtID, int nMoveType,
                double dX, double dY, double dZ, double dRx, double dRy, double dRz,
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius, 
                int nIsUseJoint,int nIsSeek, int nIOBit, int nIOState, string strCmdID);
 ...
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nErrorCode : Error code / 错误码
 *	@param Params : The error code of J1~Jn, 0 if no error / 每个轴(J1~Jn)的错误码，如果没有错误则为0
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadAxisErrorCode_nJ(unsigned int boxID, unsigned int rbtID, int& nErrorCode, JointsData &nAxisErrorCode);

/**
 *	@brief: Set default TCP (Tool Center Point) / 设置默认的工具中心点(TCP)
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param rbtID : Robot ID, 0 by default / 机器人ID号，默认值为0
 *	@param name : Name of the TCP to be set as default / 要设置为默认的TCP名称
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetDftTCP(unsigned int boxID, unsigned int rbtID,string name);

/**
 *	@brief: Calculate TCP orientation / 计算TCP姿态
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param rbtID : Robot ID, 0 by default / 机器人ID号，默认值为0
 *	@param dUCS_X-dUCS_RZ : Coordinates and rotation angles in user coordinate system / 用户坐标系下的坐标和旋转角
 *	@param dTCP_X-dTCP_RZ : Coordinates and rotation angles of TCP / TCP的坐标和旋转角
 *	@param RX-RZ : Calculated rotation angles  / 计算得到的TCP姿态
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_CalTCPOrt(unsigned int boxID, unsigned int rbtID, double dUCS_X,double dUCS_Y,double dUCS_Z,double dUCS_RX,double dUCS_RY,double dUCS_RZ,double dTCP_X,double dTCP_Y,double dTCP_Z,double dTCP_RX,double dTCP_RY,double dTCP_RZ,double &RX,double&RY,double &RZ);

/**
 *	@brief: Read current coordinates of the robot arm / 读取机械臂当前坐标
 ...
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param rbtID : Robot ID, 0 by default / 机器人ID号，默认值为0
 *	@param UcsName : Name of the user coordinate system / 用户坐标系名称
 *	@param TcpName : Name of the tool center point / 工具中心点(TCP)名称
 *	@param dJ1-dJ6 : Current joint coordinates / 当前关节坐标值
 *	@param dX-dRz : Current user coordinates / 当前用户坐标值
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActCoord(unsigned int boxID, unsigned int rbtID, string UcsName,string TcpName,double &dJ1,double &dJ2,double &dJ3,double &dJ4,double &dJ5,double &dJ6,double &dX,double &dY,double &dZ,double &dRx,double &dRy,double &dRz);

/**
 *	@brief: Read current coordinates of the robot arm (with joint data structure) / 读取机械臂当前坐标(含关节数据结构)
 *	@details: Get joint coordinates and user coordinates of the robot arm in current state by inputting TCP and UCS names / 通过输入TCP和UCS名称，获取机械臂当前状态下的关节坐标和用户坐标
 *	@param boxID : Control box ID, 0 by default / 电箱ID号，默认值为0
 *	@param rbtID : Robot ID, 0 by default / 机器人ID号，默认值为0
 *	@param UcsName : Name of the user coordinate system / 用户坐标系名称
 *	@param TcpName : Name of the tool center point / 工具中心点(TCP)名称
 *	@param joints : Structure storing current joint coordinates / 存储当前关节坐标值的结构体
 *	@param dX-dRz : Current user coordinates  / 当前用户坐标值
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Function call error code / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadActCoord_nJ(unsigned int boxID, unsigned int rbtID, string UcsName,string TcpName,JointsData& joints,double &dX,double &dY,double &dZ,double &dRx,double &dRy,double &dRz);



/**
 *	@brief: Clear the linkage extension axis position (set the current position of the linkage extension axis to zero point) / 清除联动扩展轴位置(设联动扩展轴当前位置为零点)
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ClearCoControlExpandAxisPos(unsigned int boxID, unsigned int rbtID);

/**
 *	@brief: Set the current base mounting angle. / 设定当前机座安装角度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nRotation : Set base rotation angle/设置机座在水平面上的旋转。
 *  @param nTilt: Set base tilt angle/设置机座在垂直方向上的倾斜。
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetBaseInstallingAngle(unsigned int boxID, unsigned int rbtID,int nRotation,int nTilt);

/**
 *	@brief: Get the current base mounting angle / 获取当前机座安装角度
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nRotation : Get base rotation angle /获取机座在水平面上的旋转
 *  @param nTilt: Get base tilt angle /获取机座在垂直方向上的倾斜
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_GetBaseInstallingAngle(unsigned int boxID, unsigned int rbtID,int &nRotation,int &nTilt);

/**
 *	@brief: Start Load Identification. / 开始负载识别
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_LoadIdentify(unsigned int boxID,unsigned int rbtID);

/**
 * @brief Get the load identification result./获取负载辨识结果
 *
 * @param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param status Load identification status:/负载辨识状态
 *               - 0: 成功 (Success)
 *               - 1: 第一条轨迹错误 (First trajectory error)
 *               - 2: 第二条轨迹错误 (Second trajectory error)
 *               - 3: 第三条轨迹错误 (Third trajectory error)
 *               - 4: 负载辨识运行中 (Load identification in progress)
 *               - 5: 意外停止 (Unexpected stop)
 *               - 6: 意外退出 (Unexpected exit)
 *               - 7: 初始化，负载辨识未开始 (Initialization, load identification not started)
 * @param progress: Progress status (0~100)(Current load identification progress)/ 进度状态 - 当前负载识别进度
 *                 
 * @param mass : Identified load mass (unit: kg)./负载质量,Kg
 * @param cx :Load center of mass X coordinate./质心X坐标
 * @param cy :Load center of mass Y coordinate./质心Y坐标
 * @param cz : Load center of mass Z coordinate./质心Z坐标
 */
HANSROBOT_PRO_API int HRIF_GetLoadIdentifyResult(unsigned int boxID, unsigned int rbtID,int &status,int &progress,double &mass,double &cx,double &cy,double &cz);

/**
 * @brief Get calibration parameters/设置标定参数
 * @param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param dCalib_Fx Get calibration force in x-direction / 获取x方向标定力
 * @param dCalib_Fy Get calibration force in y-direction / 获取y方向标定力（Fy0）
 * @param dCalib_Fz Get calibration force in z-direction / 获取z方向标定力（Fz0）
 * @param dCalib_Mx Get calibration moment in x-direction / 获取x方向标定力矩（Mx0）
 * @param dCalib_My Get calibration moment in y-direction / 获取y方向标定力矩（My0）
 * @param dCalib_Mz Get calibration moment in z-direction / 获取z方向标定力矩（Mz0）
 * @param dCalib_G Get gravity / 重力（G）
 * @param dCalib_X Get center of gravity offset in x-direction / 获取x方向重心偏移（X）
 * @param dCalib_Y Get center of gravity offset in y-direction/ 获取y方向重心偏移（Y）
 * @param dCalib_Z Get center of gravity offset in z-direction/ 获取z方向重心偏移（Z）
 * @param dRotateAngle Get current base mounting Installation angle / 获取当前基座安装角度（InstallRotationAngle）
 *   @return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
*/
HANSROBOT_PRO_API int HRIF_GetLastCalibParams(unsigned int boxID, unsigned int rbtID, double &dCalib_Fx,
    double &dCalib_Fy,
    double &dCalib_Fz,
    double &dCalib_Mx,
    double &dCalib_My,
...
/**
 * @brief Set calibration parameters / 设置标定参数
 * @param boxID :Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param dCalib_Fx Set calibration force in x-direction / 设置x方向标定力
 * @param dCalib_Fy Set calibration force in y-direction / 设置y方向标定力（Fy0）
 * @param dCalib_Fz Set calibration force in z-direction / 设置z方向标定力（Fz0）
 * @param dCalib_Mx Set calibration moment in x-direction / 设置x方向标定力矩（Mx0）
 * @param dCalib_My Set calibration moment in y-direction / 设置y方向标定力矩（My0）
 * @param dCalib_Mz Set calibration moment in z-direction / 设置z方向标定力矩（Mz0）
 * @param dCalib_G Set gravity / 重力（G）
 * @param dCalib_X Set center of gravity offset in x-direction / 设置x方向重心偏移（X）
 * @param dCalib_Y Set center of gravity offset in y-direction/ 设置y方向重心偏移（Y）
 * @param dCalib_Z Set center of gravity offset in z-direction/ 设置z方向重心偏移（Z）
 * @param dRotateAngle Set current base mounting Installation angle / 设置当前基座安装角度（InstallRotationAngle）
 *   @return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetInitializeForceSensor(unsigned int boxID, unsigned int rbtID,double dCalib_Fx,
        double dCalib_Fy,
        double dCalib_Fz,
        double dCalib_Mx,
        double dCalib_My,
...
/**
 * @brief  Set the calibration data for the force sensor./设置力传感器的标定数据
 *
 * @param boxID :Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param pointnum 标定点位的数量
 ...
 *              用于力控标定的点位数据
 *              - 参数总数必须为 12 的倍数，且在 96~192 之间。
 *              - 依次存放 X, Y, Z, Rx, Ry, Rz, Fx, Fy, Fz, Tx, Ty, Tz。
 *              - 每 12 个参数为一组数据，支持 8~16 组数据（即参数个数 96~192）
 *              - 前六个为位置坐标，后六个为力控数据，单位：mm、N。
 */
HANSROBOT_PRO_API int HRIF_SetFTCalibration(unsigned int boxID, unsigned int rbtID, int pointnum,vector<vector<double>> param);

/**
 * @brief Read  force data from the force sensor./读取力传感器的原始力数据
 * @param boxID :Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param dX :  Force data in the X direction./x方向的原始力数据
 * @param dY Force data in the Y direction./y方向的原始力数据
 * @param dZ :Force data in the Z direction./z方向的原始力数据
 * @param dRX :Force data in the RX direction./RX方向的原始力数据
 * @param dRY: Force data in the RY direction./ RY方向的原始力数据
 * @param dRZ :Force data in the RZ direction.RZ方向的原始力数据
 * @return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadForceData(unsigned int boxID,  unsigned int rbtID,double &dX, double &dy, double &dZ, double &dRX, double &dRY, double &dRZ);

/**
 * @brief Read  force data from the force sensor./读取力传感器的原始力数据
 * @param boxID :Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param weaveType :  Weaving type ( 0 = sine weaving; other types may be added later)./摆焊类型（int，0为正弦摆焊，后续会增加其他摆焊类型）。
 * @param frequency Oscillation frequency (double, unit: Hz)./摆动频率（double，单位：Hz）。
 * @param amplitude :Oscillation amplitude (double, unit: mm)./z方向的原始力数据
 * @param weaveFrameType :Weaving plane selection (int, 0: Plane formed by the tool coordinate system’s Z-axis and welding path direction; 1: Plane formed by the tool coordinate system’s Z-axis and Y-axis)./摆焊平面选择（int,0:由工具坐标系z方向和焊道方向构成坐标系，1：由工具坐标系z方向和y方向构成坐标系）。
 * @param rightDwell:  Right dwell time (double, unit: seconds)./ 右停留时间（double，单位：秒）。
 * @param leftDwell :Left dwell time (double, unit: seconds)./左停留时间（double，单位：秒）。
 * @param centreDwell :Centre dwell time (double, unit: seconds)./中间停留时间（double，单位：秒）。
 * @param LPatternAngle : L-pattern angle (double, unit: degrees; currently unsupported, set to 0 for now)./L型角度（double，单位：度，暂时不支持这一功能，后续增加，当前使用时应设为0）。
 * @param elevation :Elevation angle (double, unit: degrees)./仰角（double，单位：度）。
 * @param azimuth :Azimuth angle (double, unit: degrees)./方位角（double，单位：度）。
 * @param centreRise: Center rise (double, unit: mm; currently should be set to 0, support will be added later)./ 中心隆起（double，单位：mm，当前应设置为0，后续会增加对隆起的支持）。
 * @param radius :Radius (double, unit: mm)../半径（double，单位：mm）。
 * @return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */ 
HANSROBOT_PRO_API int HRIF_SetWeaveParams(unsigned int boxID, unsigned int rbtID, int weaveType, double frequency, double amplitude, int weaveFrameType, double rightDwell, double leftDwell, double centreDwell, double LPatternAngle,
    double elevation, double azimuth, double centreRise, double radius);

/**
 * @brief Start or Stop Weave Welding Operation./启动或关闭摆焊操作
 * @param boxID :Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 * @param startWeave : Weaving Welding Enable Flag (1 = ON, 0 = OFF)/启动编织焊接标志（1表示启动焊接，0表示停止焊接）。
 * @return           nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_StartWeaveWeld(unsigned int boxID,  unsigned int rbtID,int startWeave);
