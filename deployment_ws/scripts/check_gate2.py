...
def _luggage_root(repo: Path) -> Path:
    if (repo / "src" / "luggage_description").is_dir():
        return repo
    nested = repo / "elfin_humble_ws"
    if (nested / "src" / "luggage_description").is_dir():
        return nested
    return repo


LUGGAGE = _luggage_root(REPO)
MOUNT = LUGGAGE / "src" / "luggage_description" / "urdf" / "eef_sensor_mount.urdf.xacro"
ORIGIN = LUGGAGE / "src" / "luggage_description" / "config" / "mid360_origin.xacro"
CFG = LUGGAGE / "src" / "luggage_description" / "config" / "MID360s_config.json.example"
